...
/* ============================================================
   MODAL MEDIA — YouTube video + photo gallery support
   ============================================================
   Add these OPTIONAL fields to any resource in your RESOURCES list
   (data.js) to enable the video/gallery view in the download popup:
     video:   "https://www.youtube.com/watch?v=XXXXXXXXXXX"  (any
              youtube.com/watch, youtu.be, or /shorts link works)
     gallery: ["img1.jpg", "img2.jpg", "img3.jpg"]            (2-3
              extra screenshots, shown alongside the main image)
   If neither is set, the popup just falls back to the single
   r.image like before — nothing breaks for resources without them.
   ============================================================ */
...
function buildModalMediaItems(r) {
  const items = [];
  const ytId = youTubeId(r.video);
  if (ytId) {
    items.push({ type: "video", id: ytId, thumb: `https://img.youtube.com/vi/${ytId}/hqdefault.jpg` });
  }
  if (r.image) items.push({ type: "image", src: r.image });
  if (Array.isArray(r.gallery)) {
    r.gallery.forEach((src) => { if (src) items.push({ type: "image", src }); });
  }
  if (!items.length) items.push({ type: "image", src: r.image || "" });
  return items;
}

function renderModalMediaMain(item) {
  if (item.type === "video") {
    return `<iframe class="modal-media-frame" src="https://www.youtube.com/embed/${item.id}?autoplay=1&rel=0" title="Preview video" frameborder="0" allow="autoplay; encrypted-media; picture-in-picture" allowfullscreen></iframe>`;
  }
  return `<div class="modal-media-frame" style="background-image:url('${item.src}'); background-size:cover; background-position:center;"></div>`;
}

function wireModalMedia(mediaItems) {
  if (mediaItems.length <= 1) return;
  let idx = 0;
  const mainEl = document.getElementById("modal-media-main");
  const thumbs = Array.from(document.querySelectorAll(".modal-media-thumb"));

  function showIdx(i) {
    idx = (i + mediaItems.length) % mediaItems.length;
    mainEl.innerHTML = renderModalMediaMain(mediaItems[idx]);
    thumbs.forEach((t, ti) => t.classList.toggle("is-active", ti === idx));
  }

  document.getElementById("media-prev").addEventListener("click", () => showIdx(idx - 1));
  document.getElementById("media-next").addEventListener("click", () => showIdx(idx + 1));
  thumbs.forEach((t) => t.addEventListener("click", () => showIdx(Number(t.dataset.idx))));
}
...
function openModal(id) {
  ...
  const mediaItems = buildModalMediaItems(r);
  const hasMultiple = mediaItems.length > 1;

  modal.innerHTML = `
    <div class="modal-media">
      <div class="modal-media-main" id="modal-media-main">${renderModalMediaMain(mediaItems[0])}</div>
      <button class="modal-close" id="modal-close" aria-label="Close">${ICONS.close}</button>
      ${hasMultiple ? `
        <button class="media-nav media-nav-prev" id="media-prev" aria-label="Previous">${ICONS.chevL}</button>
        <button class="media-nav media-nav-next" id="media-next" aria-label="Next">${ICONS.chevR}</button>
      ` : ""}
    </div>
    ${hasMultiple ? `
      <div class="modal-media-thumbs" id="modal-media-thumbs">
        ${mediaItems.map((it, i) => `
          <button class="modal-media-thumb ${i === 0 ? "is-active" : ""}" data-idx="${i}" style="background-image:url('${it.thumb || it.src}')">
            ${it.type === "video" ? `<span class="thumb-play">${ICONS.play}</span>` : ""}
          </button>
        `).join("")}
      </div>
    ` : ""}
    <div class="modal-body">
      ...
      <div class="modal-actions">
        <button type="button" class="btn btn-violet btn-block" id="get-script-btn">${ICONS.download} ${r.linkLabel || "Get Script"}</button>
        ${previewBtn}
      </div>
    </div>
  `;
  ...
  document.getElementById("get-script-btn").addEventListener("click", () => openDownloadGate(r));
  wireModalMedia(mediaItems);
}

/* ============================================================
   DOWNLOAD GATE
   ------------------------------------------------------------
   Instead of "Get Script" linking straight out to the real
   Mediafire / Mega.nz / etc. URL, it now swaps the modal into
   this intermediate screen — showing the script name + file
   info first, with its own "Download File" button that is the
   ONLY thing that actually opens r.link.
   ============================================================ */
function openDownloadGate(r) {
  const modal = document.getElementById("modal");
  const typeText = (r.fileType || "zip").toUpperCase();
  const sizeText = r.fileSize || "";

  modal.innerHTML = `
    <button class="modal-close" id="modal-close" aria-label="Close">${ICONS.close}</button>
    <div class="gate-body">
      <div class="gate-brand">
        <img src="assets/logo.png" alt="" class="gate-brand-logo">
        <span class="gate-brand-name">EXE<span class="accent">LEAKS</span></span>
      </div>

      <div class="gate-card">
        <button type="button" class="gate-back" id="gate-back">${ICONS.chevL} Back</button>

        <div class="gate-file-row">
          <div class="gate-file-icon">${ICONS.download}</div>
          <div class="gate-file-info">
            <div class="gate-file-name">${r.name}</div>
            <div class="gate-file-meta">
              <span class="tag-chip">${typeText}</span>
              ${sizeText ? `<span class="tag-chip">${sizeText}</span>` : ""}
            </div>
          </div>
        </div>

        <div class="gate-status"><span class="gate-status-dot"></span> Ready to download</div>

        <a href="${r.link}" target="_blank" rel="noopener" class="btn btn-violet btn-block gate-download-btn" id="gate-download-btn">
  ...
  document.getElementById("modal-close").addEventListener("click", closeModal);
  document.getElementById("gate-back").addEventListener("click", () => openModal(r.id));
  document.getElementById("gate-download-btn").addEventListener("click", () => {
    r.stats.downloads = (r.stats.downloads || 0) + 1;
    SITE_STATS.totalDownloads = (SITE_STATS.totalDownloads || 0) + 1;
    renderStats();
    closeModal();
  });
}
...
function closeModal() {
  ...
  document.getElementById("modal").innerHTML = ""; // stops any video that was still playing
}
...
function wireRulesLink() {
  ...
  document.querySelectorAll(".js-rules-link").forEach((el) => {
...
function wireAboutLink() {
  ...
  document.querySelectorAll(".js-about-link").forEach((el) => {
    el.addEventListener("click", (e) => {
      e.preventDefault();
      openAboutModal();
    });
...
// figures out whether the media link is a video or an image/gif and
// returns the right tag to drop into the popup
function buildPromoMediaEl(url) {
  const clean = String(url || "").split("?")[0].toLowerCase();
  if (clean.endsWith(".mp4") || clean.endsWith(".webm")) {
    return `<video class="promo-media" src="${url}" autoplay loop muted playsinline></video>`;
  }
  return `<img class="promo-media" src="${url}" alt="">`;
}

function openPromoAd() {
  const overlay = document.getElementById("promo-overlay");
  document.getElementById("promo-media-wrap").innerHTML = buildPromoMediaEl(PROMO_AD.media);
...
/* ============================================================
   HERO BOOT TERMINAL — types out a short boot log, then settles
   on a status line. Purely decorative, respects reduced-motion.
   ============================================================ */
function initBootTerminal() {
  ...
  const finalText = "system online — onam drops unlocked";
  if (reduce) {
  ...
  const lines = [
    "connecting to exeLEAKS.mainframe…",
    "laying out the pookalam…",
    finalText
  ];
  let li = 0, ci = 0;
  ...
  function typeNext() {
    ...
    if (ci <= line.length) {
      ...
      ci++;
      setTimeout(typeNext, li === lines.length - 1 ? 32 : 18);
